...
describe('H1R9-F.5.3.2: Direct Safe Print Margins & Physical Printer Protection', () => {
  ...
  const createPlan = async (id: string) => {
    ...
    plan.days = [
      {
        date: '2026-08-24',
        dayOfWeek: 'MONDAY',
        activities: [{ activityId: 'act-mon', category: 'C', objective: 'ACTIVIDAD_LUNES_MARGIN_TEST', description: 'Desc Mon', durationMinutes: 25, materials: ['MATERIAL_LUNES_MARGIN_TEST'], curricularTraceability: [] }],
        complementaryActivities: [{ programArea: 'PROGRAMA_LUNES', activityName: 'ACTIVIDAD_COMP_LUNES' }],
        materials: ['MATERIAL_LUNES_MARGIN_TEST'],
        evaluation: 'EVALUACION_LUNES_MARGIN_TEST',
        evaluationStatus: 'APPROVED'
      },
      {
        date: '2026-08-25',
        dayOfWeek: 'TUESDAY',
        activities: [{ activityId: 'act-tue', category: 'C', objective: 'ACTIVIDAD_MARTES_MARGIN_TEST', description: 'Desc Tue', durationMinutes: 30, materials: ['MATERIAL_MARTES_MARGIN_TEST'], curricularTraceability: [] }],
        complementaryActivities: [],
        materials: ['MATERIAL_MARTES_MARGIN_TEST'],
        evaluation: 'EVALUACION_MARTES_MARGIN_TEST',
        evaluationStatus: 'APPROVED'
      },
    ...
    plan.observations = 'Observaciones Margin Test';
  ...
  it('A-I. DIRECT print renders 5 Anverso and 5 Reverso daily pairs in preserved order with full content integrity', async () => {
    await createPlan('plan-margin-test');
    await openDirectOfficialView();

    const weekdays = ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY'];

    // A & B: 5 Anversos + 5 Reversos exist
    for (const d of weekdays) {
      expect(screen.getByTestId(`direct-anverso-${d}`)).toBeDefined();
      expect(screen.getByTestId(`direct-reverso-${d}`)).toBeDefined();
    }

    // C-G: Monday Anverso integrity
    ...
    // H: Monday Reverso curricular matrix
    ...
    // I: Pair order check within root
    const root = document.getElementById('printable-document-root');
    expect(root).toBeDefined();
    const dayElements = root?.querySelectorAll('.direct-daily-document');
    expect(dayElements?.length).toBe(5);
  });

  it('J. INDIRECT official view remains behaviorally and structurally unchanged', async () => {
    await createPlan('plan-indirect-margin-test');
    render(<PlanningDemoApp service={service} source={source} />);
